...
class LJ_API_Import extends WP_Importer {
	...
	function header() {
		echo '<div class="wrap">';
		screen_icon();
		echo '<h2>' . __( 'Import LiveJournal' , 'livejournal-importer') . '</h2>';
	}
	...
	function greet() {
		...
		<?php if ( get_option( 'ljapi_username' ) && get_option( 'ljapi_password' ) && get_option( 'ljapi_website' )) : ?>
			<input type="hidden" name="step" value="<?php echo esc_attr( get_option( 'ljapi_step' ) ) ?>" />
			<p><?php _e( 'It looks like you attempted to import your LiveJournal posts previously and got interrupted.' , 'livejournal-importer') ?></p>
			<p class="submit">
				<input type="submit" class="button" value="<?php esc_attr_e( 'Continue previous import' , 'livejournal-importer') ?>" />
			</p>
			<p class="submitbox"><a href="<?php echo esc_url($_SERVER['PHP_SELF'] . '?import=livejournal&amp;step=-1&amp;_wpnonce=' . wp_create_nonce( 'lj-api-import' ) . '&amp;_wp_http_referer=' . esc_attr( $_SERVER['REQUEST_URI'] )) ?>" class="deletion submitdelete"><?php _e( 'Cancel &amp; start a new import' , 'livejournal-importer') ?></a></p>
			<p>
		<?php else : ?>
			<input type="hidden" name="step" value="1" />
			<input type="hidden" name="login" value="true" />
			<p><?php _e( 'Howdy! This importer allows you to connect directly to LiveJournal and download all your entries and comments' , 'livejournal-importer') ?></p>
			<p><?php _e( 'Enter your LiveJournal-compatible website (i.e. www.livejournal.com), username and password below so we can connect to your account:' , 'livejournal-importer') ?></p>

			<table class="form-table">

			<tr>
			<th scope="row"><label for="lj_website"><?php _e( 'LiveJournal-compatible web site URL' , 'livejournal-importer') ?></label></th>
			<td><input type="text" name="lj_website" id="lj_website" class="regular-text" /></td>
			</tr>

			<tr>
			<th scope="row"><label for="lj_username"><?php _e( 'LiveJournal Username' , 'livejournal-importer') ?></label></th>
			<td><input type="text" name="lj_username" id="lj_username" class="regular-text" /></td>
			</tr>

			<tr>
			<th scope="row"><label for="lj_password"><?php _e( 'LiveJournal Password' , 'livejournal-importer') ?></label></th>
			<td><input type="password" name="lj_password" id="lj_password" class="regular-text" /></td>
			</tr>

			</table>

			<p><?php _e( 'If you have any entries on LiveJournal which are marked as private, they will be password-protected when they are imported so that only people who know the password can see them.' , 'livejournal-importer') ?></p>
			<p><?php _e( 'If you don&#8217;t enter a password, ALL ENTRIES from your LiveJournal will be imported as public posts in WordPress.' , 'livejournal-importer') ?></p>
			...
			<p><?php _e( "<strong>WARNING:</strong> This can take a really long time if you have a lot of entries in your LiveJournal, or a lot of comments. Ideally, you should only start this process if you can leave your computer alone while it finishes the import." , 'livejournal-importer') ?></p>

			<p class="submit">
				<input type="submit" class="button" value="<?php esc_attr_e( 'Connect to LiveJournal and Import' , 'livejournal-importer') ?>" />
			</p>

			<p><?php _e( '<strong>NOTE:</strong> If the import process is interrupted for <em>any</em> reason, come back to this page and it will continue from where it stopped automatically.' , 'livejournal-importer') ?></p>

			<noscript>
				<p><?php _e( '<strong>NOTE:</strong> You appear to have JavaScript disabled, so you will need to manually click through each step of this importer. If you enable JavaScript, it will step through automatically.' , 'livejournal-importer') ?></p>
			</noscript>
		<?php endif; ?>
		</form>
		</div>
	...
	function download_post_meta() {
		...
		echo '<p>' . __( 'Post metadata has been downloaded, proceeding with posts...' , 'livejournal-importer') . '</p>';
	}
	...
	function import_post( $post ) {
		...
		if ( $post_id = post_exists( $post_title, $post_content, $post_date ) ) {
			printf( __( 'Post <strong>%s</strong> already exists.' , 'livejournal-importer'), stripslashes( $post_title ) );
		} else {
	...
	function get_session() {
		...
		if ( is_wp_error( $cookie ) )
			return new WP_Error( 'cookie', __( 'Could not get a cookie from LiveJournal. Please try again soon.' , 'livejournal-importer') );
	...
	function download_comment_meta() {
		...
		while ( $maxid > $highest_id ) {
			...
			$this->comments_url = 'http://'.$this->website.'/export_comments.bml';
			$results = wp_remote_get( $this->comments_url . '?get=comment_meta&startid=' . ( $highest_id + 1 ),
										array( 'cookies' => array( $cookie ), 'timeout' => 20 ) );
			if ( is_wp_error( $results ) )
				return new WP_Error( 'comment_meta', __( 'Failed to retrieve comment meta information from LiveJournal. Please try again soon.' , 'livejournal-importer') );
			...
			if ( 0 == $matches[1] ) {
				...
				echo '<p>' . __( 'You have no comments to import!' , 'livejournal-importer') . '</p>';
		...
		echo '<p>' . __( 'Comment metadata downloaded successfully, proceeding with comment bodies...' , 'livejournal-importer') . '</p>';
	...
	function download_comment_bodies() {
		...
		while ( $maxid > $highest_id && $loop < 5 ) { // We do 5 loops per call to avoid memory limits
			...
			$results = wp_remote_get( $this->comments_url . '?get=comment_body&startid=' . ( $highest_id + 1 ),
										array( 'cookies' => array( $cookie ), 'timeout' => 20 ) );
			if ( is_wp_error( $results ) )
				return new WP_Error( 'comment_bodies', __( 'Failed to retrieve comment bodies from LiveJournal. Please try again soon.' , 'livejournal-importer') );
			...
			// If we had no comments in this batch then find the highest ID of the deleted ones we ignored and update to that
			// Otherwise we loop the loop forever
			if ( 0 == $count ) {
				if ( 0 != preg_match_all( '|<comment id=\'(\d+)\'.*state=\'D\'[^/]*/>|iUs', $results, $matches ) ) {
					$max_deleted_id = array_pop( $matches[1] );
					update_option( 'ljapi_highest_comment_id', $max_deleted_id );
				}
			}
	...
	function parse_comment( $comment ) {
		...
		$author = ( empty( $comment_author_ID ) || empty( $this->usermap[$comment_author_ID] ) || substr( $this->usermap[$comment_author_ID], 0, 4 ) == 'ext_' ) ? __( 'Anonymous' , 'livejournal-importer') : $this->usermap[$comment_author_ID];
		if ( get_option( 'ljapi_username' ) == $author ) {
			...
			$user_id = 0;
			$url     = ( __( 'Anonymous' , 'livejournal-importer') == $author ) ? '' : 'http://' . $author . '.livejournal.com/';
		}
	...
	function lj_ixr() {
		...
		if ( isset( $challenge['challenge'] ) ) {
			...
			return new WP_Error( 'IXR', __( 'LiveJournal is not responding to authentication requests. Please wait a while and then try again.' , 'livejournal-importer') );
		}
		...
		if ( $this->ixr->query( 'LJ.XMLRPC.' . $method, $params ) ) {
			...
			return new WP_Error( 'IXR', __( 'XML-RPC Request Failed -- ' , 'livejournal-importer') . $this->ixr->getErrorCode() . ': ' . $this->ixr->getErrorMessage() );
		}
	}
	...
	function setup() {
		...
		if ( empty( $this->website ) || empty( $this->username ) || empty( $this->password ) ) {
			?>
			<p><?php _e( 'Please enter your LiveJournal-compatible website, username <em>and</em> password so we can download your posts and comments.' , 'livejournal-importer') ?></p>
			<p><a href="<?php echo esc_url($_SERVER['PHP_SELF'] . '?import=livejournal&amp;step=-1&amp;_wpnonce=' . wp_create_nonce( 'lj-api-import' ) . '&amp;_wp_http_referer=' . esc_attr( str_replace( '&step=1', '', $_SERVER['REQUEST_URI'] ) ) ) ?>"><?php _e( 'Start again' , 'livejournal-importer') ?></a></p>
			<?php
			return false;
		}
		...
		if ( is_wp_error( $verified ) ) {
			if ( 100 == $this->ixr->getErrorCode() || 101 == $this->ixr->getErrorCode() ) {
				...
				delete_option( 'ljapi_website' );
				delete_option( 'ljapi_protected_password' );
				?>
				<p><?php _e( 'Logging in to LiveJournal failed. Check website url, your username and password and try again.' , 'livejournal-importer') ?></p>
				<p><a href="<?php echo esc_url($_SERVER['PHP_SELF'] . '?import=livejournal&amp;step=-1&amp;_wpnonce=' . wp_create_nonce( 'lj-api-import' ) . '&amp;_wp_http_referer=' . esc_attr( str_replace( '&step=1', '', $_SERVER['REQUEST_URI'] ) ) ) ?>"><?php _e( 'Start again' , 'livejournal-importer') ?></a></p>
				<?php
				return false;
			} else {
...
#			$this->_create_ixr_client();
		...
		echo '<h3>' . __( 'Importing Posts' , 'livejournal-importer') . '</h3>';
		echo '<p>' . __( 'We&#8217;re downloading and importing your LiveJournal posts...' , 'livejournal-importer') . '</p>';
		if ( get_option( 'ljapi_post_batch' ) && count( get_option( 'ljapi_sync_item_times' ) ) ) {
			...
			echo '<p><strong>' . sprintf( __( 'Imported post batch %d of <strong>approximately</strong> %d' , 'livejournal-importer'), ( get_option( 'ljapi_post_batch' ) + 1 ), $batch ) . '</strong></p>';
		}
		ob_flush(); flush();
		...
		if ( is_wp_error( $result ) ) {
			if ( 406 == $this->ixr->getErrorCode() ) {
				?>
				<p><strong><?php _e( 'Uh oh &ndash; LiveJournal has disconnected us because we made too many requests to their servers too quickly.' , 'livejournal-importer') ?></strong></p>
				<p><strong><?php _e( 'We&#8217;ve saved where you were up to though, so if you come back to this importer in about 30 minutes, you should be able to continue from where you were.' , 'livejournal-importer') ?></strong></p>
				<?php
				echo $this->next_step( 1, __( 'Try Again' , 'livejournal-importer') );
				return false;
			} else {
				$this->throw_error( $result, 1 );
				return false;
		...
		?>
			...
			<p><input type="submit" class="button" value="<?php esc_attr_e( 'Import the next batch' , 'livejournal-importer') ?>" /> <span id="auto-message"></span></p>
			</form>
			<?php $this->auto_ajax( 'ljapi-auto-repost', 'auto-message' ); ?>
		<?php
		} else {
			echo '<p>' . __( 'Your posts have all been imported, but wait &#8211; there&#8217;s more! Now we need to download &amp; import your comments.' , 'livejournal-importer') . '</p>';
			echo $this->next_step( 2, __( 'Download my comments &raquo;' , 'livejournal-importer') );
			$this->auto_submit();
		}
		echo '</div>';
	}
	...
	function step2() {
		...
		echo '<div id="ljapi-status">';
		echo '<h3>' . __( 'Downloading Comments&#8230;' , 'livejournal-importer') . '</h3>';
		echo '<p>' . __( 'Now we will download your comments so we can import them (this could take a <strong>long</strong> time if you have lots of comments)...' , 'livejournal-importer') . '</p>';
		ob_flush(); flush();
		...
		?>
			<form action="admin.php?import=livejournal" method="post" id="ljapi-auto-repost">
			<p><strong><?php printf( __( 'Imported comment batch %d of <strong>approximately</strong> %d' , 'livejournal-importer'), get_option( 'ljapi_comment_batch' ), $batch ) ?></strong></p>
			<?php wp_nonce_field( 'lj-api-import' ) ?>
		...
		<?php
			...
			echo '<p>' . __( 'Your comments have all been imported now, but we still need to rebuild your conversation threads.' , 'livejournal-importer') . '</p>';
			echo $this->next_step( 3, __( 'Rebuild my comment threads &raquo;' , 'livejournal-importer') );
			$this->auto_submit();
		}
		echo '</div>';
	}
	...
	function step3() {
		...
		echo '<div id="ljapi-status">';
		echo '<h3>' . __( 'Threading Comments&#8230;' , 'livejournal-importer') . '</h3>';
		echo '<p>' . __( 'We are now re-building the threading of your comments (this can also take a while if you have lots of comments)...' , 'livejournal-importer') . '</p>';
		ob_flush(); flush();
		...
		if ( $imported_comments > 1 )
			echo '<p>' . sprintf( __( "Successfully re-threaded %s comments." , 'livejournal-importer'), number_format( $imported_comments ) ) . '</p>';
		echo '<h3>';
		printf( __( 'All done. <a href="%s">Have fun!</a>' , 'livejournal-importer'), get_option( 'home' ) );
		echo '</h3>';
		echo '</div>';
	}
	...
	function auto_submit( $id = 'ljapi-next-form', $msg = 'auto-message', $seconds = 10 ) {
		?><script type="text/javascript">
			...
			function ljapi_msg() {
				str = '<?php echo esc_js( _e( "Continuing in %d&#8230;" , 'livejournal-importer') ); ?>';
				jQuery( '#<?php echo $msg ?>' ).html( str.replace( /%d/, next_counter ) );
				if ( next_counter <= 0 ) {
					if ( jQuery( '#<?php echo $id ?>' ).length ) {
						jQuery( "#<?php echo $id ?> input[type='submit']" ).hide();
						str = '<?php echo esc_js( __( "Continuing&#8230;" , 'livejournal-importer') ); ?> <img src="<?php echo esc_url( admin_url( 'images/wpspin_light.gif' ) ); ?>" alt="" id="processing" align="top" />';
	...
	function auto_ajax( $id = 'ljapi-next-form', $msg = 'auto-message', $seconds = 5 ) {
		?><script type="text/javascript">
			...
			function ljapi_msg() {
				str = '<?php echo esc_js( __( "Continuing in %d&#8230;" , 'livejournal-importer') ); ?>';
				jQuery( '#<?php echo $msg ?>' ).html( str.replace( /%d/, next_counter ) );
				if ( next_counter <= 0 ) {
					if ( jQuery( '#<?php echo $id ?>' ).length ) {
						jQuery( "#<?php echo $id ?> input[type='submit']" ).hide();
						jQuery.ajaxSetup({'timeout':3600000});
						str = '<?php echo esc_js( __( "Processing next batch." , 'livejournal-importer') ); ?> <img src="<?php echo esc_url( admin_url( 'images/wpspin_light.gif' ) ); ?>" alt="" id="processing" align="top" />';
						jQuery( '#<?php echo $msg ?>' ).html( str );
	...
	function _create_ixr_client() {
		...
		if ( !$this->ixr ) {
			...
			$this->ixr->useragent = 'WordPress/' . $wp_version . '; LiveJournal Importer - ' . get_bloginfo( 'url' );
		}
	}
}
